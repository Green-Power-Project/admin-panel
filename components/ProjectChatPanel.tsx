...
export default function ProjectChatPanel({
  ...
  const handleEditSave = async () => {
    if (!editingMessageId || editingValue.trim() === '') return;
    try {
      await updateMessageText(projectId, editingMessageId, editingValue.trim());
        ...
        <div className="relative flex flex-col w-full max-w-lg bg-white shadow-xl ml-auto h-full">
          ...
          {replyTo && (
            <div className="px-4 py-2 bg-gray-100 border-t border-gray-200 flex items-center justify-between">
              <span className="text-sm text-gray-600 truncate flex-1">{t('chat.replyLabel')} {replyTo.text || (replyTo.fileType === 'pdf' ? 'PDF' : 'Image')}</span>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-4xl max-h-[90vh] w-full overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-4 py-2 border-b">
              ...
              <div className="flex gap-2">
                ...
                <button type="button" onClick={() => setViewerFile(null)} className="text-gray-500 hover:text-gray-700">
                  {t('common.close')}
                </button>
              </div>
            </div>
            <div className="flex-1 min-h-0 overflow-auto p-4">
              {viewerFile.type === 'pdf' ? (
                <iframe src={viewerFile.url} className="w-full h-[70vh] border-0 rounded" title={t('chat.pdfViewer')} />
              ) : (
...
function MessageBubble({
  ...
  const canEdit = isAdminPanel && isOwn && msg.text != null && !msg.fileUrl;
  const canDelete = isAdminPanel;
        ...
        <div className={`flex items-center gap-1 text-xs ${isOwn ? 'text-green-600' : 'text-slate-500'}`}>
          ...
          {isOwn && (
            <span
              ...
              title={msg.status === 'read' ? t('chat.read') : t('chat.sent')}
            >
              {msg.status === 'read' ? '✓✓' : '✓'}
            </span>
          )}
          ...
          {isEditing ? (
            <div className="space-y-2">
              ...
              <div className="flex gap-2 justify-end">
                <button type="button" onClick={onEditCancel} className="text-xs text-gray-600 hover:underline">
                  {t('common.cancel')}
                </button>
                <button type="button" onClick={onEditSave} className="text-xs text-green-600 font-medium hover:underline">
                  {t('common.save')}
                </button>
              </div>
            </div>
            ...
            <>
              ...
              {msg.text && (
                <p className="text-sm whitespace-pre-wrap break-words">
                  {msg.text}
                  {msg.editedAt != null && <span className="ml-1 text-xs opacity-75">({t('chat.edited')})</span>}
                </p>
              )}
              {isFile && (
          ...
          {menuOpen && (
            <div className="absolute top-full right-0 mt-1 min-w-[140px] py-1 bg-white border border-gray-200 rounded-lg shadow-lg z-50">
              ...
              <button type="button" onClick={() => onCopy()} className="w-full px-3 py-2 text-left text-sm text-gray-700 hover:bg-gray-100 flex items-center gap-2">
                {copied ? t('chat.copied') : <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>{t('chat.copy')}</>}
              </button>
              {canEdit && (
                <button type="button" onClick={onEdit} className="w-full px-3 py-2 text-left text-sm text-gray-700 hover:bg-gray-100 flex items-center gap-2">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                  {t('chat.edit')}
                </button>
              )}
              {canDelete && (
                <button type="button" onClick={onDelete} className="w-full px-3 py-2 text-left text-sm text-red-600 hover:bg-red-50 flex items-center gap-2">
