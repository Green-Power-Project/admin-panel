...
export function subscribeToLatestMessage(
  ...
  const messagesRef = ref(realtimeDb, messagesPath(projectId));
  const latestOnly = query(messagesRef, orderByKey(), limitToLast(1));
  const unsubscribe = onValue(latestOnly, (snapshot) => {
    const val = snapshot.val();
    if (!val || typeof val !== 'object') {
      onMessage(null);
      return;
    ...
    const entries = Object.entries(val) as [string, Record<string, unknown>][];
    const sorted = entries.sort((a, b) => (a[1].createdAt as number) - (b[1].createdAt as number));
...
export function subscribeToLatestAdminMessage(
  ...
  const messagesRef = ref(realtimeDb, messagesPath(projectId));
  const limited = query(messagesRef, orderByKey(), limitToLast(MESSAGE_LIMIT));
  const unsubscribe = onValue(limited, (snapshot) => {
    const val = snapshot.val();
    const list: ChatMessage[] = [];
    if (val && typeof val === 'object') {
